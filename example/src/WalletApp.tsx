...
export class WalletApp extends React.Component<any, any> {
  ...
  public connect = async () => {
    ...
    const connector = new WalletProvider({ qrcodeModal: QRCodeModal, bridge })
  ...
  public subscribeToEvents = (connector) => {
    ...
    connector.on('session_update', async (error, payload) => {
      ...
      this.onSessionUpdate(connector)
    })
  ...
  public onSessionUpdate = async (connector) => {
    const { chainId, accounts } = connector
    const address = accounts[0]
  ...
  public testSendTransaction = async () => {
    const { address, chainId } = this.state
    const connector = this.state.connector.connector
    if (!connector) {
      return
    }
